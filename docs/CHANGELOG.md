...
## 2026-08-10 — パスワードと二要素認証をアプリから変えられるようにした

- Progress A7。今までアプリからは、パスワードの変更も二要素認証の設定も
  できず、ブラウザで PhotoNest を開く必要があった。ADR-0010 は A7 を
  「自分のライブラリへの入り口を保つ手当」として範囲の内側に置いている。
  **サーバー側の変更は無い**（`PUT /api/auth/profile` と `/api/auth/2fa/*` は
  既にあった）。

### ログイン側も同時に直した

- **有効にしただけでは、次のログインで締め出されていた。** サーバーの
  `POST /api/auth/login` は、認証アプリが登録済みのアカウントに対して
  `401 totp_required` を返すが、アプリは `token` を送らず、ログイン画面にも
  コードの入力欄が無かった。サインアウト・再インストール・別端末のいずれでも
  入れなくなる。**二要素を有効にできるようにする以上、ログインの対応が
  前提**なので同じ変更に含めた。
- コード欄は**サーバーが要求してから**出す。持っていない人に埋められない欄を
  最初から見せない。一度出したら、コードが違っても引っ込めない
  （引っ込めると成功しようのないフォームへ戻る）。
- `otpauth` を `AndroidManifest.xml` の `<queries>` へ追加した。API 30 以降は
  ここに無いスキームがパッケージ可視性で隠れ、`canLaunchUrl` が
  「開けない」と答える——認証アプリが入っていても「認証アプリで開く」が
  必ず空振りしていた。

### 設定タブから 1 画面

- 設定タブのアカウントカードに「パスワードと二要素認証」を足し、1 タップで
  `/account` を開く。**タブも他の画面も増やしていない。**
- パスワードは 2 回入力させる。見えない入力を 1 回で確定させると、打ち間違いが
  他の全端末からの締め出しになる。前後の空白は削らない——ログインも削らないので、
  ここで削ると本人が入れなくなる。
- 二要素は「秘密鍵を取得 → 認証アプリのコードで確定」の 2 段。取得しただけでは
  サーバー側は何も変わらず、途中で閉じればアカウントはそのまま。登録は
  `otpauth://` を外部アプリで開くのが第一で、セットアップキー（コピー可）と
  QR も併記する（QR は別の端末から読むためのもの）。
- 有効・無効の切り替えは、サーバーの応答をそのまま画面へ反映する。書き込みの
  後に読み直さない——その読み込みが失敗すると、成功した変更がエラー画面の裏に
  隠れる。

### 入れなかったもの

- メールアドレス・表示名・パスキー。表示名は**そもそも読み出せない**
  （`GET /api/auth/me` は未設定のときメールアドレスを返すため、「未設定」と
  「メールアドレスと同じ」を区別できない）。パスキーは端末側の WebAuthn 対応が
  要るので切り離し、画面には「ブラウザから設定する」ことだけを 1 行書いた。
  判断は `docs/adr/0014-account-screen-covers-credentials-only.md`。
