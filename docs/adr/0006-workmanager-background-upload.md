...
- 同期パスは前提条件（機能 ON・ログイン済み・ライブラリ権限）を毎回
  再検査する。フォアグラウンドとバックグラウンドは別アイソレートで
  メモリ内フラグを共有できないため、履歴の読み取り〜アップロード〜履歴
  書き込みまでを共有 SQLite 上の同期リース（`sync_leases` テーブル、
  `SyncLeaseRepository`）で相互排他する。リースは 30 分で自然失効し、
  クラッシュした保持者が以後のパスを塞ぐことはない。リースが取れなかった
