...
| 3 | 21 | ライブラリ全体を時系列で見る画面がない（閲覧はアルバム経由のみ） | ⬜未着手 | 大 | 大 | 中 | 大 |
| 4 | 22 | 全画面ビューアで前後の写真へ移動できず、原本も見られない | ⬜未着手 | 中 | 中 | 小 | 中 |
| 5 | 23 | アップロードの進捗が枚数単位で、失敗の詳細がアプリ再起動で消える | ⬜未着手 | 小 | 中 | 中 | 中 |
| 6 | 24 | iOS 版がない（`ios/` ディレクトリごと存在しない） | 🟡要判断 | 大 | 中 | 大 | 大 |
