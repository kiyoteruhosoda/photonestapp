...
このアプリはサーバーの薄いクライアントではなく、機内モードでも直近の状態を
出せるところまで持つ。そのぶん「どちらの値を正とするか」を決めておかないと、
端末ごとに違うものが見える。現在の取り決めは以下。

- **書き込みの結果はサーバーが確定した値を採る。** お気に入りの切り替えは
  押した側の期待値ではなく応答の値を画面へ書き戻す。別の端末が先に変えて
  いた場合、こちらの操作は「見えている状態から反転」ではなく
  「保存されている状態を表示」で終わる。
- **タグは差分ではなく全置換で送る**（エンドポイントがそうなっている）。
  タグエディタは保存を押すまで何も送らないので、開いている間に別端末が同じ
  メディアのタグを変えていると、後から保存した側の集合で上書きされる。
- **絞り込みが変わったタイムラインは先頭の窓から読み直す。**
  `libraryMediaQueryProvider` が変わると、前の絞り込みで取ったカーソルは別の
  結果集合を指すため、継ぎ足しでは順序が壊れる。検索欄の待ちは
  `mediaSearchDebounce`。
- **ゴミ箱からの復元はタイムラインを触らない。** 復元したメディアが撮影日時順
  のどこへ入るかを推測せず、次の読み込みに任せる。ゴミ箱の一覧
  （`trashedMediaProvider`）は `autoDispose` で、画面を出るたびに忘れる
  （まもなく完全に消えるものの一覧を持ち続けても古くなるだけのため）。
- **サムネイルは一括発行の署名付き URL で取る。** `ThumbnailUrlBatch` が同じ
  フレームで求められたぶんを 1 リクエスト（`POST /api/media/thumb-urls`）に
  束ねる。発行できない・失敗する・URL が期限切れの場合はアプリサーバー経由
  （`GET /api/media/{id}/thumbnail`）へ落ちるため、単発の経路も残す。

ローカル DB のファイル名は `flutterbase.db`（`AppDatabase.fileName`）のまま。
端末内部の名前で画面にも API にも出ないため、改名しても得は無く、既存
